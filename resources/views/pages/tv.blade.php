...
@section('content')
    ...
    <div class="mb-4 flex items-center justify-between">
      ...
      <div class="relative">
        <select id="movie-filter"
          ...
          onchange="filterTv(this.value)">
          <option value="all" class="bg-gray-700">All</option>
          <option value="airing_today" class="bg-gray-700">Airing Today</option>
          <option value="on_the_air" class="bg-gray-700">On The Air</option>
    ...
    @foreach ($tvShows as $tv)
      ...
      <div class="group relative overflow-hidden rounded-lg">
        <a href="/tv/{{ $tv->id }}" class="relative aspect-[2/3]">
          <img src="{{ $tmdb_imageBaseUrl }}/w500/{{ $tv->poster_path }}" alt="{{ $tv->name }}"
  ...
  <script>
    ...
    function createTvElement(item) {
      ...
      const tvYear = new Date(release_date).getFullYear();
    ...
    function processResponse(response) {
      if (response.results) {
        return response.results.map(createTvElement).join('');
      }
      return '';
    }
    ...
    function loadMore() {
      const url =
        `${baseURL}/discover/tv?api_key=${apiKey}&sort_by=popularity.desc&vote_count.gte=${minimalVoter}&page=${++currentPage}`;
    ...
    function filterTv(filter) {
      ...
      // const urlApi = `${baseURL}/tv/${filter}?api_key=${apiKey}&language=en-US&page=${filterMoviePage}`;
      const urlApi = filter == 'all' ?
        `${baseURL}/discover/tv?api_key=${apiKey}&sort_by=popularity.desc&vote_count.gte=${minimalVoter}&page=${++currentPage}` :
        `${baseURL}/tv/${filter}?api_key=${apiKey}&language=en-US&page=${filterMoviePage}`;
